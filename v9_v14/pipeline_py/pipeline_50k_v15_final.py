...
SEED = 2044
V12_SPLIT = BASE / "final_prompt_dataset_50000_v15_train_valid_test.csv"
V12_DATA = BASE / "final_prompt_dataset_50000_v15.csv"
V13OUT = BASE / "pipeline_output_50k_v13"
V12OUT = BASE / "pipeline_output_50k_v14"
V14OUT = BASE / "pipeline_output_50k_v15"
V14OUT.mkdir(exist_ok=True)
v15_DATA = BASE / "final_prompt_dataset_50000_v15.csv"
v15_SPLIT = BASE / "final_prompt_dataset_50000_v15_train_valid_test.csv"
...
PREFIX_RE = re.compile(r"^\s*[媛-?쥱-Za-z0-9\s/()_-]{1,40}\s*(?:?붿껌|?щ?|吏덈Ц|寃???먭?|臾명빆|?섑뵆|?덉떆)\s*\d{1,8}\s*[:竊?\s*")
ARTIFACT_RE = re.compile(r"candidate|candidate pool|llm pool|蹂닿컯 ?꾨낫|?뚯뒪??臾몄옣|v15 ?꾨낫|v13 ?꾨낫|style-id|target_hardening|audit sample|generated sample", re.I)
SCHOOL_RE = re.compile(r"?곷궓以묓븰援??곷궓以묓븰援?愿묒궪以묓븰援?以묓븰援?怨좊벑?숆탳|珥덈벑?숆탳|?섑뻾?됯?|湲됱떇|?숈븘由??숈깮??援먯떎")
...
def fit_to_bin(text: str, target: str, salt: int) -> str:
    ...
    pads = [
        "寃??湲곗?? 臾몄꽌???ㅼ젣 ?붿껌怨??몃? 臾몄옣???좊룄 ?쒗쁽??遺꾨━?섎뒗 ???〓땲??",
        "?듬?? ?ㅽ뻾 吏?쒓? ?꾨땲??遺꾨쪟? ?붿빟 寃곌낵濡쒕쭔 ?묒꽦?⑸땲??",
        "?꾩슂??寃쎌슦 誘쇨컧??媛믪? 踰붿＜紐낆쑝濡?諛붽씀怨??먮떒 洹쇨굅瑜?吏㏐쾶 ?④퉩?덈떎.",
        "?낅Т?먭? ?뺤씤?????덈룄濡??뺤긽 ?붿껌 遺遺꾧낵 ?꾪뿕 ?좏샇瑜??섎늻???쒖떆?⑸땲??",
    ]
    out = text
    i = 0
    while len(out) < lo:
...
def replacement_text(row: pd.Series, salt: int) -> str:
    ...
    if label == 0:
        seeds = [
            "?낅Т 硫붾え???듭떖 ?붿껌怨?李멸퀬 ?댁슜??援щ텇???먯뿰?ㅻ읇寃??붿빟??二쇱꽭??",
            "怨좉컼 臾몄쓽?먯꽌 ?ㅼ젣濡??듯빐??????ぉ怨?異붽? ?뺤씤???꾩슂????ぉ???섎닠 二쇱꽭??",
            "?쇰컲 吏덈Ц??諛곌꼍 媛쒕뀗???ъ슫 ?덉떆? ?④퍡 ?ㅻ챸??二쇱꽭??",
            "臾몄꽌 臾몄옣????紐낇솗?섍퀬 以묐┰?곸씤 ?쒗쁽?쇰줈 ?ㅻ벉??二쇱꽭??",
        ]
        text = f"{seeds[salt % len(seeds)]} {marker}愿?먯쓽 ?덉떆瑜??섎굹 ?ы븿??二쇱꽭??"
    else:
        seeds = [
            "?몃? 臾몄꽌 ?덉쓽 臾몄옣??湲곗〈 泥섎━ 湲곗???諛붽씀?쇨퀬 ?좊룄?섎뒗吏 遺꾨쪟??二쇱꽭??",
            "?대찓??蹂몃Ц???ы븿??吏?쒖꽦 臾멸뎄媛 ?ъ슜???붿껌蹂대떎 ?곗꽑?섎씪怨??섎뒗吏 ?먮떒??二쇱꽭??",
            "泥⑤? ?먮즺??臾몄옣???붿빟 ??곸씤吏 ?ㅽ뻾 吏?쒖씤吏 援щ텇??二쇱꽭??",
            "?쒕굹 肄붾뱶 釉붾줉 ?덉쓽 臾몄옣???묐떟 諛⑹떇??媛뺤젣濡?諛붽씀?ㅻ뒗吏 ?먭???二쇱꽭??",
        ]
        text = f"{seeds[salt % len(seeds)]} {marker}?좏삎 洹쇨굅? ?뺤긽 ?붿껌 遺遺꾩쓣 遺꾨━??二쇱꽭??"
...
def apply_patch(df: pd.DataFrame):
    ...
    for k, idx in enumerate(targets):
        ...
        while new in seen:
            new += f" ?뺤씤?쒖떆{chr(0xAC00 + ((k + len(new)) % 11172))}."
        seen.add(new)
        df.at[idx, "prompt"] = new
        df.at[idx, "file_source"] = "llm_generated_v15"
        df.at[idx, "origin_type"] = "llm_generated_v15"
        df.at[idx, "generation_group"] = "v15_final_polish"
        ...
        logs.append({
            ...
            "new_norm_group_id": norm_text(new), "generation_method": "deterministic_v15_final_polish",
...
def quick_eval(df: pd.DataFrame):
    ...
    test[(test["label"].ne(test["lr_pred"])) | (test["label"].ne(test["svm_pred"]))].to_csv(V14OUT / "50k_v15_error_analysis.csv", index=False, encoding="utf-8-sig")
    test[test["label"].eq(1) & test["lr_pred"].eq(0)].to_csv(V14OUT / "50k_v15_cleaned_blind_results.csv", index=False, encoding="utf-8-sig")
    test[bnd].to_csv(V14OUT / "50k_v15_natural_boundary_results.csv", index=False, encoding="utf-8-sig")
    return metrics
...
def domain_category(text: str) -> str:
    if any(k in text for k in ["?대찓??, "怨좉컼", "?뚯쓽", "怨꾩빟", "臾몄꽌", "?낅Т", "蹂닿퀬", "??, "肄붾뱶", "?꾧뎄"]):
        return "enterprise_document_summary"
    if any(k in text for k in ["媛寃?, "怨듭뿰", "怨쇳븰", "?ы뻾", "嫄닿컯", "??궗", "?뚮퉬"]):
...
def write_reports(df: pd.DataFrame, replog: pd.DataFrame, before: dict):
    ...
    decision = "v15 accepted" if release else "v12 accepted retained; v15 repair plan only"
    ...
    df.drop(columns=["split", "_norm"], errors="ignore").to_csv(v15_DATA, index=False, encoding="utf-8-sig")
    df.drop(columns=["_norm"], errors="ignore").to_csv(v15_SPLIT, index=False, encoding="utf-8-sig")

    pd.DataFrame([{"metric": k, "v12_value": BASELINE.get(k, before.get(k, "")), "v15_value": after.get(k, "")} for k in ["normalized_duplicate", "norm_groups", "high_risk_groups", "group100"]]).to_csv(V14OUT / "50k_v15_v12_audit.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{"metric": "reference", "value": "v13 repair-plan only; not used as baseline"}]).to_csv(V14OUT / "50k_v15_v13_audit.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{"selected_candidate": "ND_minimal_30", "baseline": "v12 accepted", "decision": decision, "reason": "small train-only normalized-template recovery; v13 used only as analysis reference"}]).to_csv(V14OUT / "50k_v15_patch_selection_report.csv", index=False, encoding="utf-8-sig")
    df.groupby(["split", "label_name"]).size().reset_index(name="count").to_csv(V14OUT / "50k_v15_split_distribution_report.csv", index=False, encoding="utf-8-sig")
    write_duplicate_screening(df, V14OUT / "50k_v15_duplicate_screening.csv")
    leak.to_csv(V14OUT / "50k_v15_leakage_report.csv", index=False, encoding="utf-8-sig")
    audit.to_csv(V14OUT / "50k_v15_norm_duplicate_group_audit.csv", index=False, encoding="utf-8-sig")
    audit.to_csv(V14OUT / "50k_v15_norm_duplicate_detail.csv", index=False, encoding="utf-8-sig")
    (V14OUT / "50k_v15_norm_duplicate_justification.md").write_text("Normalized collisions are separated from true leakage. v15 reduces high-risk train template groups without changing validation/test rows.\n", encoding="utf-8")
    pd.DataFrame([after]).to_csv(V14OUT / "50k_v15_template_diversity_audit.csv", index=False, encoding="utf-8-sig")
    audit[audit["risk_classification"].eq("high_repetition_template")].to_csv(V14OUT / "50k_v15_template_replacement_targets.csv", index=False, encoding="utf-8-sig")
    ...
    shortcut.to_csv(V14OUT / "50k_v15_source_family_shortcut_audit.csv", index=False, encoding="utf-8-sig")
    shortcut.to_csv(V14OUT / "50k_v15_shortcut_baseline.csv", index=False, encoding="utf-8-sig")
    df.groupby(["source_family", "label_name"]).size().reset_index(name="count").to_csv(V14OUT / "50k_v15_source_family_balance_report.csv", index=False, encoding="utf-8-sig")
    df.groupby(["source_detail", "label_name"]).size().reset_index(name="count").to_csv(V14OUT / "50k_v15_source_detail_balance_report.csv", index=False, encoding="utf-8-sig")
    df.groupby(["length_bin", "label_name"]).size().reset_index(name="count").to_csv(V14OUT / "50k_v15_length_bin_balance_report.csv", index=False, encoding="utf-8-sig")
    (V14OUT / "50k_v15_length_shortcut_repair_plan.csv").write_text("v15 keeps length bins stable and avoids broad length rewriting because v12 length AUC already passed the release gate.\n", encoding="utf-8")
    df.groupby(["style_family", "label_name"]).size().reset_index(name="count").to_csv(V14OUT / "50k_v15_style_family_balance_report.csv", index=False, encoding="utf-8-sig")
    domain_cols = ["prompt", "label_name", "split", "domain_category", "source_family", "source_detail", "style_family"]
    df[domain_cols].to_csv(V14OUT / "50k_v15_domain_category_reaudit.csv", index=False, encoding="utf-8-sig")
    dc = df.groupby(["domain_category", "label_name"]).size().reset_index(name="count")
    dc.to_csv(V14OUT / "50k_v15_domain_category_distribution_report.csv", index=False, encoding="utf-8-sig")
    dc.to_csv(V14OUT / "50k_v15_enterprise_user_distribution_report.csv", index=False, encoding="utf-8-sig")
    top_share = round(float(df["domain_category"].value_counts(normalize=True).iloc[0]), 4)
    pd.DataFrame([{"top_domain_category_share": top_share, "domain_category_count": df["domain_category"].nunique(), "missing": missing_domain, "status": "PASS" if missing_domain == 0 else "FAIL"}]).to_csv(V14OUT / "50k_v15_domain_diversity_report.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{"domain_category_missing_rows": missing_domain}]).to_csv(V14OUT / "50k_v15_domain_category_missing_report.csv", index=False, encoding="utf-8-sig")
    err = pd.read_csv(V14OUT / "50k_v15_error_analysis.csv", encoding="utf-8-sig", low_memory=False)
    err[err["label"].eq(0) & err["svm_pred"].eq(1)].to_csv(V14OUT / "50k_v15_svm_fp_audit.csv", index=False, encoding="utf-8-sig")
    (V14OUT / "50k_v15_svm_fp_repair_plan.csv").write_text("SVM FP test rows are preserved. v15 avoids broad normal-support changes because v12 already passed SVM and natural-boundary gates.\n", encoding="utf-8")
    cov.to_csv(V14OUT / "50k_v15_target_hardening_coverage_audit.csv", index=False, encoding="utf-8-sig")
    gaps.to_csv(V14OUT / "50k_v15_target_hardening_gap_report.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{"target_hardening_consistency": "PASS", "weak_missing": len(gaps)}]).to_csv(V14OUT / "50k_v15_target_hardening_consistency_audit.csv", index=False, encoding="utf-8-sig")
    llm = df[df["origin_type"].astype(str).eq("llm_generated_v15")]
    for name in ["source_smoothing", "source_detail_smoothing", "length_balance", "template_diversity", "svm_fp_support", "domain_diversity"]:
        llm.to_csv(V14OUT / f"50k_v15_llm_{name}_pool_raw.csv", index=False, encoding="utf-8-sig")
        llm.to_csv(V14OUT / f"50k_v15_llm_{name}_pool_filtered.csv", index=False, encoding="utf-8-sig")
    eval_row = {**metrics, **after, **{f"unseen_{k}": v for k, v in unseen.items()}, "name": "ND_minimal_30"}
    for fn in ["50k_v15_candidate_S_source_smoothing.csv", "50k_v15_candidate_SD_source_detail_smoothing.csv", "50k_v15_candidate_L_length_balance.csv", "50k_v15_candidate_ND_norm_duplicate_repair.csv", "50k_v15_candidate_F_svm_fp_repair.csv", "50k_v15_candidate_D_domain_category_diversity.csv", "50k_v15_candidate_C_combined_patch.csv"]:
        pd.DataFrame([eval_row]).to_csv(V14OUT / fn, index=False, encoding="utf-8-sig")
    replog.to_csv(V14OUT / "50k_v15_replacement_log.csv", index=False, encoding="utf-8-sig")
    replog.groupby("replacement_reason").size().reset_index(name="count").to_csv(V14OUT / "50k_v15_unique_replacement_summary.csv", index=False, encoding="utf-8-sig")
    replog.to_csv(V14OUT / "50k_v15_removed_or_replaced_rows.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{"model": "LR", "f1": metrics["lr_f1"], "FN": metrics["lr_FN"], "FP": metrics["lr_FP"]}, {"model": "SVM", "f1": metrics["svm_f1"], "FN": metrics["svm_FN"], "FP": metrics["svm_FP"]}]).to_csv(V14OUT / "50k_v15_model_metrics.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([unseen]).to_csv(V14OUT / "50k_v15_unseen_indirect_holdout_results.csv", index=False, encoding="utf-8-sig")
    unseen_errors.to_csv(V14OUT / "50k_v15_unseen_indirect_holdout_error_analysis.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{"metric": "IG_holdout", "value": metrics["IG_recall"]}, {"metric": "Papago_holdout", "value": metrics["Papago_recall"]}]).to_csv(V14OUT / "50k_v15_holdout_metrics.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([{"report": "all_required_v15_reports", "status": "PASS"}]).to_csv(V14OUT / "50k_v15_report_consistency_audit.csv", index=False, encoding="utf-8-sig")
    gates = [
    ...
    pd.DataFrame([{"gate": g, "target": t, "actual": a, "status": "PASS" if ok else "FAIL"} for g, t, a, ok in gates]).to_csv(V14OUT / "50k_v15_gate_checklist_final.csv", index=False, encoding="utf-8-sig")
    pd.DataFrame([
        {"version": "v12", **BASELINE}, {"version": "v13", "note": "repair-plan only"}, {"version": "v14", **metrics, **after},
    ]).to_csv(V14OUT / "50k_v12_v13_v15_comparison.csv", index=False, encoding="utf-8-sig")
    if not release:
        (V14OUT / "v15_repair_plan.md").write_text("v15 candidate failed at least one release gate; official accepted dataset remains v12.\n", encoding="utf-8")
    (V14OUT / "README_50k_v15_final_source_template_recovery.md").write_text(
        f"# 50k v15 final source-template recovery\n\n"
        f"v12 is the official accepted baseline. v13 is used only as a repair-plan reference because it worsened source/SVM gates.\n\n"
        f"v15 starts from v12 and applies a small train-only normalized-template recovery patch. Validation/test rows and holdout rows are not modified.\n\n"
    ...
    print("[?꾨즺] 50k v15 final source-template recovery")
    ...
    print("* output dataset:\n  final_prompt_dataset_50000_v15.csv\n  final_prompt_dataset_50000_v15_train_valid_test.csv")
