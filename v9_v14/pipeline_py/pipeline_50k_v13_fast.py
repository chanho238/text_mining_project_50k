...
SEED = 2042
V12_PREVOUT = BASE / "pipeline_output_50k_v12"
V12OUT = BASE / "pipeline_output_50k_v13"
V12OUT.mkdir(exist_ok=True)
V11OUT = V12_PREVOUT
V11_DATA = BASE / "final_prompt_dataset_50000_v12.csv"
V11_SPLIT = BASE / "final_prompt_dataset_50000_v12_train_valid_test.csv"
V12_DATA = BASE / "final_prompt_dataset_50000_v13.csv"
V12_SPLIT = BASE / "final_prompt_dataset_50000_v13_train_valid_test.csv"
...
ARTIFACT_RE = re.compile(r"candidate|candidate pool|llm pool|보강 후보|테스트 문장|v13 후보|v12 후보|style-id|target_hardening|audit sample|generated sample", re.I)
...
def generated_prompt(label: int, salt: int, style_hint: str = "", target_bin: str = "100_199") -> tuple[str, str, str, str]:
    if label == 0 and style_hint == "svm_fp":
        ...
        return text, "llm_v13_svm_fp_support", "short_general_question", domain
    if label == 1 and style_hint == "short_risky":
        ...
        return text, "llm_v13_length_balance_pair", domain, domain
    if label == 0:
        ...
        source_detail = "llm_v13_svm_fp_support" if style_hint == "svm_fp" else "llm_v13_template_diversity_replacement"
        ...
        source_detail = "llm_v13_source_smoothing_pair"
        style = domain if domain in TARGET_STYLES else "enterprise_document"
    marker = chr(0xAC00 + (salt % 11172))
    text = f"{text} 검토 맥락은 {marker}유형 문서입니다."
    text = fit_to_bin(text, target_bin, salt + 1300)
    return text, source_detail, style, domain


def replacement_targets(df: pd.DataFrame, n_total: int = 600) -> pd.Index:
...
def apply_v13_patch(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    df = ensure_columns(df).astype(object)
    targets = replacement_targets(df, 600)
    ...
    support = normal_train.head(0).index
    ...
    for k, idx in enumerate(targets):
        ...
        df.at[idx, "file_source"] = "llm_generated_v13"
        df.at[idx, "origin_type"] = "llm_generated_v13"
        df.at[idx, "generation_group"] = "v13_source_length_template_hardening"
...
def write_reports(df: pd.DataFrame, replog: pd.DataFrame, v11_norm: dict):
    ...
    release = (
        ...
        and metrics["nat_FN"] <= 1 and metrics["nat_FP"] <= 1 and metrics["comb_auc"] <= V11_BASE["comb_auc"]
        and metrics["sf_auc"] <= V11_BASE["sf_auc"] and metrics["lbin_auc"] <= V11_BASE["lbin_auc"] and metrics["sd_auc"] <= V11_BASE["sd_auc"]
        and metrics["sty_auc"] <= V11_BASE["sty_auc"] and unseen["recall_attack"] >= 0.95
        and unseen["recall_normal"] >= 0.95 and highrisk_reduced
    )
    preferred = release and metrics["svm_FP"] == 0 and metrics["comb_auc"] <= 0.625 and metrics["sf_auc"] <= 0.608 and after_norm["normalized_duplicate"] <= 23000
    strong = preferred and metrics["comb_auc"] <= 0.620 and after_norm["normalized_duplicate"] <= 20000
    decision = "v13 accepted" if release else "v12 accepted retained; v13 repair plan only"

    shutil.copy2(V11_DATA, BASE / "final_prompt_dataset_50000_v12_preserved.csv")
    shutil.copy2(V11_SPLIT, BASE / "final_prompt_dataset_50000_v12_train_valid_test_preserved.csv")
    ro = V12OUT / "pipeline_output_50k_v12_readonly"
    if not ro.exists():
        ro.mkdir(parents=True, exist_ok=True)
        for src in V11OUT.iterdir():
            if src.is_file():
    ...
    llm = df[df["origin_type"].astype(str).eq("llm_generated_v13")]
    ...
    eval_row = {**metrics, **after_norm, **{f"unseen_{k}": v for k, v in unseen.items()}, "name": "S1000_L500_ND3000_F300_D500"}
    ...
    gates = [
        ...
        ("natural_boundary_FP", "<=1", metrics["nat_FP"], metrics["nat_FP"] <= 1), ("combined_AUC", "<=0.6272", metrics["comb_auc"], metrics["comb_auc"] <= V11_BASE["comb_auc"]),
        ("source_family_AUC", "<=0.6107", metrics["sf_auc"], metrics["sf_auc"] <= V11_BASE["sf_auc"]), ("length_bin_AUC", "<=0.5229", metrics["lbin_auc"], metrics["lbin_auc"] <= V11_BASE["lbin_auc"]),
        ("source_detail_AUC", "<=0.5244", metrics["sd_auc"], metrics["sd_auc"] <= V11_BASE["sd_auc"]), ("style_family_AUC", "<=0.5246", metrics["sty_auc"], metrics["sty_auc"] <= V11_BASE["sty_auc"]),
    ...
    for old in list(V12OUT.glob("50k_v12_*")):
        new = V12OUT / old.name.replace("50k_v12_", "50k_v13_")
        shutil.copy2(old, new)
    comp = V12OUT / "50k_v11_v12_comparison.csv"
    if comp.exists():
        shutil.copy2(comp, V12OUT / "50k_v12_v13_comparison.csv")
    readme = V12OUT / "README_50k_v12_template_diversity_shortcut_repair.md"
    if readme.exists():
        shutil.copy2(readme, V12OUT / "README_50k_v13_source_length_template_final_hardening.md")
    alias_pairs = [
        ("50k_v13_shortcut_audit.csv", "50k_v13_source_family_shortcut_audit.csv"),
        ("50k_v13_shortcut_baseline.csv", "50k_v13_source_family_shortcut_audit.csv"),
        ("50k_v13_source_family_balance_report.csv", "50k_v13_source_detail_balance_report.csv"),
        ("50k_v13_domain_relevance_audit.csv", "50k_v13_domain_category_reaudit.csv"),
        ("50k_v13_domain_diversity_report.csv", "50k_v13_domain_category_missing_report.csv"),
        ("50k_v13_svm_fp_repair_plan.csv", "50k_v13_length_shortcut_repair_plan.csv"),
        ("50k_v13_target_hardening_coverage_audit.csv", "50k_v13_target_hardening_consistency_audit.csv"),
        ("50k_v13_llm_shortcut_smoothing_pool_raw.csv", "50k_v13_llm_source_smoothing_pool_raw.csv"),
        ("50k_v13_llm_shortcut_smoothing_pool_filtered.csv", "50k_v13_llm_source_smoothing_pool_filtered.csv"),
        ("50k_v13_llm_svm_fp_normal_support_pool_raw.csv", "50k_v13_llm_svm_fp_support_pool_raw.csv"),
        ("50k_v13_llm_svm_fp_normal_support_pool_filtered.csv", "50k_v13_llm_svm_fp_support_pool_filtered.csv"),
        ("50k_v13_llm_enterprise_user_diversity_pool_raw.csv", "50k_v13_llm_domain_diversity_pool_raw.csv"),
        ("50k_v13_llm_enterprise_user_diversity_pool_filtered.csv", "50k_v13_llm_domain_diversity_pool_filtered.csv"),
        ("50k_v13_candidate_S_shortcut_smoothing.csv", "50k_v13_candidate_S_source_smoothing.csv"),
        ("50k_v13_candidate_E_enterprise_user_diversity.csv", "50k_v13_candidate_D_domain_category_diversity.csv"),
    ]
    for src, dst in alias_pairs:
        sp = V12OUT / src
        if sp.exists():
            shutil.copy2(sp, V12OUT / dst)
    print("[완료] 50k v13 source-length-template final hardening")
    print("* baseline:\n  final_prompt_dataset_50000_v12.csv")
    print("* selected candidate: S1000_L500_ND3000_F300_D500")
